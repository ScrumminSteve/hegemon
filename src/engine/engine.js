...
export const RULES_REVISION = 12; // 12: B12 — no retreat query for a destroyed or siege-only attacking force (chooser branch); ghosts are repelled, not routed. 11: duplicate leave-control is free (no double charge, Rules p.24). 10: sea rally PLACEABLE again (owner ruling: dumb-but-legal, null at resolution; bot avoidance is scoring) — reverts 9. 9: CP banned at sea. 8: combat marches supply-checked at declaration, full-survival basis (Rules p.8, FAQ). 7: replacePortShips supply check (Rules p.8). 6: support terrain gate (Rules p.18) — land cannot back sea battles, ports cannot back land. 5: Rules p.12 Not-Enough-Order-Tokens
