...
# ---------- sea lanes routed over water (owner finding, Jul 2026) ----------
# Any edge touching a sea gets an A* route over the DOWNSAMPLED water grid:
# sea-sea lanes treat land as nearly impassable; sea-land lanes may make
# landfall only near their destination (land cost low but real). Routes that
# meaningfully deviate from straight are emitted to src/data/edgeRoutes.js
# for map-view to draw — printed-board sea lanes, computed from the same
# mask the map is painted from.
def compute_sea_routes(mask_img):
    import heapq
    F = 8
    arr = (np.array(mask_img) > 128)
    gh, gw = arr.shape[0] // F, arr.shape[1] // F
    land = arr[:gh * F, :gw * F].reshape(gh, F, gw, F).any(axis=(1, 3))
    def cell(x, y): return (min(gh - 1, max(0, int(y // F))), min(gw - 1, max(0, int(x // F))))
    def astar(a, b, land_cost):
        start, goal = cell(*a), cell(*b)
        openq = [(0.0, start)]; gc = {start: 0.0}; came = {}
        while openq:
            ...
            for dy in (-1, 0, 1):
                for dx in (-1, 0, 1):
                    ...
                    ng = gc[cur] + math.hypot(dx, dy) * (land_cost if land[ny, nx] else 1)
                    if ng < gc.get((ny, nx), 1e18):
                        gc[(ny, nx)] = ng; came[(ny, nx)] = cur
                        heapq.heappush(openq, (ng + math.hypot(goal[0] - ny, goal[1] - nx), (ny, nx)))
        if goal not in came and goal != start: return None
        path = [goal]
        while path[-1] != start: path.append(came[path[-1]])
        path.reverse()
        return [((cx + 0.5) * F, (cy + 0.5) * F) for cy, cx in path]
    def seg_d(pt, a, b):
        (px2, py2), (ax2, ay2), (bx2, by2) = pt, a, b
        dx, dy = bx2 - ax2, by2 - ay2
        t = max(0, min(1, ((px2 - ax2) * dx + (py2 - ay2) * dy) / (dx * dx + dy * dy or 1)))
        return math.hypot(px2 - (ax2 + t * dx), py2 - (ay2 + t * dy))
    def rdp(ptsq, eps):
        if len(ptsq) < 3: return ptsq
        dmax, idx = 0, 0
        for i in range(1, len(ptsq) - 1):
            d = seg_d(ptsq[i], ptsq[0], ptsq[-1])
            if d > dmax: dmax, idx = d, i
        if dmax <= eps: return [ptsq[0], ptsq[-1]]
        return rdp(ptsq[:idx + 1], eps)[:-1] + rdp(ptsq[idx:], eps)
    routes = {}
    for a, b in edges:
        ka, kb = kind.get(a), kind.get(b)
        if 'maritime' not in (ka, kb): continue
        pa, pb = P(*pos[a]), P(*pos[b])
        path = astar(pa, pb, 90 if (ka == 'maritime' and kb == 'maritime') else 14)
        if not path: continue
        path = rdp([pa] + path[1:-1] + [pb], 5.5 * S)
        if max((seg_d(q, pa, pb) for q in path), default=0) < 16 * S: continue  # basically straight
        routes[f'{a}|{b}'] = [[round(x / S + OX, 1), round(y / S + OY, 1)] for x, y in path]
    out = ("// GENERATED by tools/build-map.py — sea lanes routed over water via A*\n"
           "// on the land mask. Do not edit; re-run the build after map changes.\n"
           "export const EDGE_ROUTES = " + json.dumps(routes) + ";\n")
    (ROOT / 'src' / 'data' / 'edgeRoutes.js').write_text(out)
    print(f'sea lanes: {len(routes)} routed over water -> src/data/edgeRoutes.js')
...
def river_layer(mask_img, style):
    ...
    for bar in cfg.get('barriers', []):
        ...
        if bar.get('mouth') in pos:
            # Estuary (owner, Jul 2026): the river is "part of" a named sea —
            # extend the end nearer that sea until it reaches open water.
            sx, sy = P(*pos[bar['mouth']])
            end = min([0, -1], key=lambda k: math.hypot(pts[k][0] - sx, pts[k][1] - sy))
            ex, ey = pts[end]
            dxm, dym = sx - ex, sy - ey
            nm = math.hypot(dxm, dym) or 1
            mpx = mask_img.load()
            ext = []
            for stp in range(1, int(340 * S / (9 * S)) + 1):
                t2 = stp * 9 * S
                wig = math.sin(stp * 0.8) * 6 * S
                px2 = ex + dxm / nm * t2 - dym / nm / nm * 0  # along-course
                x2 = ex + dxm / nm * t2 + (-dym / nm) * wig
                y2 = ey + dym / nm * t2 + (dxm / nm) * wig
                if not (0 <= x2 < W and 0 <= y2 < H): break
                ext.append((x2, y2))
                if mpx[int(x2), int(y2)] == 0: break  # reached the bay
            pts = (list(reversed(ext)) + pts) if end == 0 else (pts + ext)
